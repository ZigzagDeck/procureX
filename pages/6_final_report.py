...
def generate_pdf_report(requirement_text, top_supplier, alternatives, spent_usd) -> bytes:
    """Generate a clean executive PDF report using FPDF with safe string encoding."""
    pdf = FPDF()
    ...
    pdf.cell(0, 12, _clean_str("ProcureX Procurement Report"), new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.set_font("Helvetica", "I", 10)
    pdf.cell(0, 6, _clean_str(f"Generated: {datetime.now(timezone.utc).strftime('%B %d, %Y %H:%M UTC')}"), new_x="LMARGIN", new_y="NEXT", align="C")
    pdf.ln(8)
    
    # Requirement
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, _clean_str("1. Requirement Prompt:"), new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.multi_cell(0, 6, _clean_str(requirement_text))
    pdf.ln(6)
    
    # Primary Recommendation
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, _clean_str("2. Primary Supplier Recommendation:"), new_x="LMARGIN", new_y="NEXT")
    ...
    pdf.cell(0, 6, _clean_str(f"Candidate: {top_name}"), new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, _clean_str(f"Website / Link: {web_url}"), new_x="LMARGIN", new_y="NEXT")
    ...
    if best_price is not None:
        pdf.cell(0, 6, _clean_str(f"Unit Price: Rs. {best_price:.1f}/pc"), new_x="LMARGIN", new_y="NEXT")
    if min_moq is not None:
        pdf.cell(0, 6, _clean_str(f"Minimum Order Quantity (MOQ): {min_moq:,} units"), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(6)
    
    # Alternatives
    if alternatives:
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, _clean_str("3. Candidate Alternatives:"), new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 10)
        for i, (alt, _) in enumerate(alternatives, start=2):
            alt_url = alt.website or (alt.source_urls[0] if alt.source_urls else "N/A")
            pdf.cell(0, 6, _clean_str(f"#{i} {alt.name} - Link: {alt_url}"), new_x="LMARGIN", new_y="NEXT")
        pdf.ln(6)
        
    # Audit summary
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, _clean_str("4. Research Audit Summary:"), new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 6, _clean_str(f"Micro-Query Cost Spent: ${spent_usd:.3f} USD"), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(10)
    
    pdf.set_font("Helvetica", "I", 9)
    pdf.cell(0, 6, _clean_str("Generated by ProcureX Autonomous B2B Procurement Intelligence Agent - An APEX Creation"), new_x="LMARGIN", new_y="NEXT", align="C")
